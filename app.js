...
async function base64ToBlob(base64Str) {
...
async function saveTracks() {
  const tracksToSave = tracks.map((track) => ({
    ...
    totalSecs: track.totalSecs,
  }));
  
  localStorage.setItem('yvre-tracks', JSON.stringify(tracksToSave));
}
...
async function saveAlbumData() {
  ...
  localStorage.setItem('yvre-album', JSON.stringify(albumToSave));
}

async function loadTracks() {
  const saved = localStorage.getItem('yvre-tracks');
  if (saved) {
    try {
      ...
      console.error('Erreur lors du chargement des morceaux:', e);
    }
  }
}

async function loadAlbumData() {
  const saved = localStorage.getItem('yvre-album');
  if (saved) {
    try {
      const parsed = JSON.parse(saved);
      albumData.name = parsed.name || 'Sound Collection';
      albumData.cover = parsed.cover || '';
    } catch (e) {
      console.error('Erreur lors du chargement de l\'album:', e);
    }
  }
}
...
function renderTrackList() {
  ...
  if (visibleTracks.length === 0) {
    ...
    list.innerHTML = visibleTracks.map(({ track: t, index: i }) => {
      ...
      // Ajout bouton corbeille admin
      const deleteBtn = accountType === 'admin' ? `<button class="track-delete-btn" data-index="${i}" title="Supprimer"><span style="font-size:18px;">🗑️</span></button>` : '';
      return `
        <div class="track-item${i === currentTrackIndex ? ' active' : ''}" data-index="${i}">
          ...
          <div class="track-actions">${deleteBtn}</div>
        </div>
      `;
    }).join('');
    ...
    document.querySelectorAll('.track-item').forEach(el => {
      el.addEventListener('click', (e) => {
        // Ne pas ouvrir le player si clic sur corbeille
        if (e.target.closest('.track-delete-btn')) return;
        openPlayer(Number(el.dataset.index));
      });
    });
    ...
    if (accountType === 'admin') {
      document.querySelectorAll('.track-delete-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          const track = visibleTracks[idx].track;
          if (confirm(`Supprimer le morceau "${track.title}" ?`)) {
            try {
              await window.supabaseApi.deleteTrack(track.id);
              await syncTracksFromCloud();
            } catch (err) {
...
async function createAlbumFromForm() {
  ...
  let cover = '';
  if (coverFile) {
    // Vérifie que l'API Supabase est bien chargée
    if (!window.supabaseApi || typeof window.supabaseApi.uploadFileToStorage !== 'function') {
      alert("Supabase n'est pas prêt. Recharge la page ou vérifie le chargement du script supabase-client.js");
      return;
    }
    try {
      cover = await window.supabaseApi.uploadFileToStorage(coverFile, 'covers');
    } catch (e) {
      alert('Erreur lors de l\'upload de la cover: ' + (e.message || e));
      return;
    }
  }
...
// CUSTOM TRACKS
// =====================
async function addTrackFromForm() {
  ...
  try {
    src = await window.supabaseApi.uploadFileToStorage(audioFile, 'tracks');
  } catch (e) {
    alert('Erreur lors de l\'upload du fichier audio: ' + (e.message || e));
    return;
  }
  ...
  await saveTracks();
  renderTrackList();
  // Ajout dans la base Supabase
  try {
    await addTrackToCloud(newTrack);
  } catch (e) {
    alert('Erreur lors de l\'enregistrement sur Supabase: ' + e.message);
  }
...
// SUPABASE CLOUD SYNC
// =====================
async function syncTracksFromCloud() {
  const data = await window.supabaseApi.fetchTracks();
  tracks.length = 0;
  // On s'assure que chaque track a bien tous les champs attendus
  data.forEach(track => {
    tracks.push({
      id: track.id,
      title: track.title,
...
async function addTrackToCloud(track) {
  await window.supabaseApi.addTrack(track);
  await syncTracksFromCloud();
}
...
async function initApp() {
  ...
  // Charger les données persistantes locales
  await loadAlbumData();

  // Charger les musiques depuis Supabase (toujours à jour)
  try {
    await syncTracksFromCloud();
  } catch (e) {
    // Si Supabase ne répond pas, fallback sur localStorage
    await loadTracks();
    console.error('Erreur lors du chargement depuis Supabase, fallback local:', e);
  }
